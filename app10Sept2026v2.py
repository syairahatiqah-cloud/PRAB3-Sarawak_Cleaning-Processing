...
remove_nonpositive_wl = st.checkbox(
    "Treat zero and negative values as missing (water level only)",
    value=(data_type == "Water Level"),
    help=(
        "Enable this for a water-level column. Every value equal to 0 or below "
        "0 will be converted to NaN before the missing-data summary and "
...
# WL quality control: zero and negative readings are invalid and must be
...
with tab1:
    ...
    st.caption(
        "This plot preserves the observations before negative and zero "
    ...
    # --------------------------------------------------------
    # Plot B: observations after negative/zero WL removal
    # --------------------------------------------------------
    if remove_nonpositive_wl:
        st.divider()
        st.subheader("B. After Removing Negative and Zero WL Values")
        st.caption(
            f"Removed {int(negative_wl_mask.sum())} negative value(s) and "
            f"{int(zero_wl_mask.sum())} zero value(s). These positions are shown "
            "as gaps and are treated as missing in the following analysis."
        )
        ...
        cleaned_fig.update_layout(
            title=(
                f"{data_type} After Removing Negative and Zero Values: "
        ...
        st.download_button(
            ...
            file_name=f"{val_col}_after_removing_negative_and_zero.html",
            mime="text/html"
        )
        ...
        st.image(
            cleaned_png,
            caption="Time Series After Removing Negative and Zero WL Values",
            use_container_width=True
        )
        ...
        st.download_button(
            ...
            data=cleaned_png,
            file_name=f"{val_col}_after_removing_negative_and_zero.png",
            mime="image/png"
        )
    else:
        st.info(
            "Enable ‘Treat zero and negative values as missing’ above to generate "
