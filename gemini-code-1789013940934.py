...
# - TAB 1: Original raw time series + time series after WL quality control
#          (Plotly + HTML + PNG via Matplotlib)
...
def bar_png_bytes(
    ...
    3) labelling non-zero / meaningful missing percentages and 0%.
    """
    x = list(x)
    ...
    # ✅ Add labels for 0.0% and meaningful Missing % values.
    # This avoids clutter while specifically keeping zeros and significant gaps.
    if show_value_labels:
        for b, h in zip(bars, y):
            if np.isnan(h) or (h > 0 and h < label_min_value):
                continue
            ax.text(
...
# Detect the selected variable before cleaning so WL-specific quality-control
# options are not applied accidentally to streamflow or another variable.
y_label = infer_yaxis_label(val_col)
data_type = (
    "Water Level"
    if y_label.startswith("Water Level")
    else ("Streamflow" if y_label.startswith("Streamflow") else "Value")
)

remove_invalid_wl = st.checkbox(
    "Treat values less than 0.5 (including zero and negative) as missing (water level only)",
    value=(data_type == "Water Level"),
    help=(
        "Enable this for a water-level column. Every value less than 0.5 "
        "will be converted to NaN before the missing-data summary and "
        "imputation steps. Leave it off for streamflow or other variables."
    )
)
...
# Preserve the observations before the WL-specific negative/zero filter.
# This separate copy is used for the original raw plot in Tab 1.
raw_series = series.copy()

# WL quality control: zero, negative, and values < 0.5 are invalid and must be
# treated as missing values. Keep separate masks so the app reports exactly
# how many values were removed in each category.
negative_wl_mask = pd.Series(False, index=series.index)
zero_wl_mask = pd.Series(False, index=series.index)
below_half_wl_mask = pd.Series(False, index=series.index)

if remove_invalid_wl:
    negative_wl_mask = series.lt(0) & series.notna()
    zero_wl_mask = series.eq(0) & series.notna()
    below_half_wl_mask = series.gt(0) & series.lt(0.5) & series.notna()
    series = series.mask(negative_wl_mask | zero_wl_mask | below_half_wl_mask)
...
if remove_invalid_wl:
    st.write(f"Negative water-level values converted to missing: {int(negative_wl_mask.sum())}")
    st.write(f"Zero water-level values converted to missing: {int(zero_wl_mask.sum())}")
    st.write(f"Water-level values > 0 and < 0.5 converted to missing: {int(below_half_wl_mask.sum())}")
...
# TABS
# ============================================================
tab1, tab2, tab3, tab4, tab5 = st.tabs(["1) Raw & Cleaned Plots", "2) Missing Summary", "3) FSM Imputation", "4) Linear Interpolation", "5) Polynomial Interpolation"])

# ============================================================
# TAB 1) Original raw plot + plot after removing negative/zero WL
# ============================================================
with tab1:
    st.header("1) Original and Quality-Controlled Time Series")
    ...
    display_mode = st.selectbox(
        ...
        index=0,
        help=(
            "The selected display is applied to both plots for a direct comparison. "
            "It changes only the displayed plots; the missing summary and imputation "
            "continue to use the cleaned observations at their original resolution."
        )
    )

    # --------------------------------------------------------
    # Plot A: original observations before negative/zero removal
    # --------------------------------------------------------
    st.subheader("A. Original Raw Data")
    st.caption(
        "This plot preserves the observations before negative, zero, or <0.5 "
        "water-level values are removed. Standard missing markers are shown as gaps."
    )
    raw_plot_x, raw_plot_y = prepare_plot_series(
        df[time_col], raw_series, display_mode
    )
    ...
    raw_fig.update_layout(
        title=f"Original Raw {data_type}: {val_col} ({display_mode})",
        xaxis_title="Date and Time",
        yaxis_title=y_label,
        hovermode="x unified"
    )
    ...
    # --------------------------------------------------------
    # Plot B: observations after negative/zero/<0.5 WL removal
    # --------------------------------------------------------
    if remove_invalid_wl:
        st.divider()
        st.subheader("B. After Removing Invalid WL Values (< 0.5)")
        st.caption(
            f"Removed {int(negative_wl_mask.sum())} negative value(s), "
            f"{int(zero_wl_mask.sum())} zero value(s), and "
            f"{int(below_half_wl_mask.sum())} value(s) between 0 and 0.5. "
            "These positions are shown as gaps and are treated as missing in the following analysis."
        )

        cleaned_plot_x, cleaned_plot_y = prepare_plot_series(
            df[time_col], series, display_mode
        )

        cleaned_fig = go.Figure()
        cleaned_fig.add_trace(go.Scatter(
            x=cleaned_plot_x,
            y=cleaned_plot_y,
            mode="lines",
            name=f"After WL quality control — {display_mode}",
            connectgaps=False,
            line=dict(width=1.2, color="green")
        ))
        cleaned_fig.update_layout(
            title=(
                f"{data_type} After Removing Values < 0.5: "
        ...
        st.plotly_chart(cleaned_fig, use_container_width=True)

        st.download_button(
            "Download plot after WL quality control (HTML)",
            data=cleaned_fig.to_html(include_plotlyjs="cdn").encode("utf-8"),
            file_name=f"{val_col}_after_removing_invalid_wl.html",
            mime="text/html"
        )

        cleaned_png = line_png_bytes(
            x=cleaned_plot_x,
            y_list=[cleaned_plot_y.to_numpy()],
            labels=[f"After WL quality control — {display_mode}"],
            title=(
                f"{data_type} After Removing Values < 0.5: "
                f"{val_col} ({display_mode})"
            ),
        ...
        st.image(
            cleaned_png,
            caption="Time Series After Removing Invalid WL Values",
            use_container_width=True
        )

        st.download_button(
            "Download plot after WL quality control (PNG)",
            data=cleaned_png,
            file_name=f"{val_col}_after_removing_invalid_wl.png",
            mime="image/png"
        )
    else:
        st.info(
            "Enable ‘Treat values less than 0.5... as missing’ above to generate "
            "the second water-level quality-control plot."
        )
